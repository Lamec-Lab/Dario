classdef calculations 
	...
	% ----
	% h4lfling Notes:
	% - Timoshenko might not be validated.
    % - Multilayer is currently incorrect. 
	% - {Rho_natFreq,E_natFreq,slendernessR_natFreq} not working (check <menu.m> for details).
    % ----
    properties                                                             
    ...
    function self = FEM_setup(self,inp)
        ...
        for i = 0:self.data(inp).n_c-1
            ...
            for j = 1:size(segments, 1)
                ...
                % Adiciona os valores Ã s estruturas
        ...
        elseif self.data(inp).noty == 2 % periodic
            ...
            % Total de elementos: soma dos elementos por cÃ©lula * nÃºmero de cÃ©lulas unitÃ¡rias
            ...
            % Repete os comprimentos das cÃ©lulas unitÃ¡rias para preencher toda a viga
    ...
    function self = Coupling(self)
        ...
        for i = 1:length(self.data)
            if i == 1
                % first beam
                BC_c{i} = self.data(i).BC; % Ãšltimo nÃ³ livre
            else
                % other beams
                BC_c{i} = [self.data(i).BC(2:end, 1) + self.data(i-1).BC(2:end, 1)-1,...
                      self.data(i).BC(2:end, 2:3)]; % Ignorar o primeiro nÃ³
            end
        end
    ...
    function self = ShapeModes(self,BT,modes)
        ...
        %         "DisplayName",sprintf('%dÂº mode',mode_cnt))
    ...
    function self = E_natFreq(self,modes,data,type)
            ...
            ylabel('\omega (Hz)')
            axis('tight')
            title('Natural Frequency x YoungÂ´s modulus')
            legend('show','Location','northeast')
    ...
    % by masthierryi
    % h4lfling dug several holes here
    function self = Freq_spectra(self, BT, nmr, prismatic_freqs) 
        modelTypes = {'Euler-Bernoulli', 'Rayleigh', 'Shear', 'Timoshenko'};
        model = modelTypes{BT};  

        % Current periodic beam frequencies
        freqs = self.result.natfreq(1:nmr);
        maxfreq = freqs(end) + (freqs(end) - freqs(end-1));

        % Bandgap search algorithm (by masthierryi)
        % h4lfling notes:
        %     This approach is largely incorrect and WILL yield incorrect results.
        %     The user is encouraged to finetune the analysis on a case-by-case
        %     scenario, by changing the divisor on:
        %         if dfreq(i) < (dfreq(i-1)/2)
        %     The correct approach involves comparing frequency points against
        %     those of a reference prismatic beam.
        %     Then the relative difference should be calculated.
        %     masthierryi chose the easy way out on this one.
        % ----
        dfreq = diff(freqs); 
        gap_indices = [];bandgap_starts = [];bandgap_ends = [];
        for i = 2:length(dfreq)
            if dfreq(i) < (dfreq(i-1)/2)
                gap_indices = [gap_indices, i-1];
                bandgap_starts = [bandgap_starts; freqs(i-1)];
                bandgap_ends = [bandgap_ends; freqs(i)];
            end
        end

        if ~isempty(bandgap_starts)
        ...
        % Plot bandgaps first (so they appear behind the points)
        if ~isempty(gap_indices)
            % Create a dummy patch for legend (not displayed)
            bandgap_dummy = fill(NaN, NaN, 'blue', 'FaceAlpha', 0.1, 'EdgeColor', 'none', 'DisplayName', 'Bandgaps');

            for i = 1:length(bandgap_starts)
                % Plot each bandgap without legend entries
                fill([0, nmr+1, nmr+1, 0], [bandgap_starts(i), bandgap_starts(i), bandgap_ends(i), bandgap_ends(i)], ...
                     'blue', 'FaceAlpha', 0.1, 'EdgeColor', 'none', 'HandleVisibility', 'off');

                % Add gap value text
                if ~isempty(gapvalue)
                    str = {sprintf('%.2f', gapvalue(i))};
                    text(2, (bandgap_starts(i) + bandgap_ends(i))/2, str, 'Color', 'black', ...
                         'HorizontalAlignment', 'left', 'FontWeight', 'bold');
                end
            end
        end

        % Plot periodic beam frequencies
        scatter(1:nmr, freqs, 25, 'blue', 'o', 'filled', 'MarkerEdgeColor', 'blue', 'LineWidth', 1.2);

        % Plot prismatic beam frequencies
        if nargin >= 4 && ~isempty(prismatic_freqs)
            scatter(1:min(nmr, length(prismatic_freqs)), prismatic_freqs(1:min(nmr, length(prismatic_freqs))), ...
                    25, 'red', 'o', 'MarkerEdgeColor', 'red', 'LineWidth', 1.2);
        end

        if nargin >= 4 && ~isempty(prismatic_freqs) && ~isempty(gap_indices)
            legend("Bandgaps", "Frequencies (Periodic)", "Frequencies (Prismatic)", ...
                   "Location", "southeast")
        elseif nargin >= 4 && ~isempty(prismatic_freqs)
            legend("Frequencies (Periodic)", "Frequencies (Prismatic)", "Location", "southeast")
        elseif ~isempty(gap_indices)
            legend("Bandgaps", "Frequencies (Periodic)", "Location", "southeast")
        else
            legend("Bandgaps", "Location", "southeast")
        end
    ...
    % h4lfling has been known to the function below                   
    function self = Periodic_Saturation(self,modes,data)
        ...
        alpha = 1/16:1/16:15/16; % h4lfling was here
        nmr = size(alpha,2); 
        ...
        for j = 1:nmr
            ...
            for inp = 1:size(data,2)
                ...
                % DO COMMENT THE CONFIG NOT NEEDED BELOW!
                self.data(inp).segments = [
                    % [rho, E, nu,        d1, d2, form,       section_length, n_elements]

                    7850, 205e9, 0.3,   0.01, 0.0250, 2,    LS2/10, 4; % simpa stepped
                    7850, 205e9, 0.3,   0.01, 0.0375, 2,    LS1/10, 4;
                    7850, 205e9, 0.3,   0.01, 0.0250, 2,    LS2/10, 4;

                    % 2700,  69e9, 0.3,   0.01, 0.0250, 2,    LS2/10, 4; %1 simpa bimat
                    % 7850, 205e9, 0.3,   0.01, 0.0250, 2,    LS1/10, 4;
                    % 2700,  69e9, 0.3,   0.01, 0.0250, 2,    LS2/10, 4;
                    ];
                % self.data(inp).segments = self.data.segments; % does not work :/
        ...
        el2 = plot(alpha, frequencies, '-s', 'LineWidth', 1); % Plot lines
        
        % Reverse legend generation
        ...
        xticks(alpha)
        xticklabels(string(1:15));
        ax = gca;
        txt = text(1.1, 0, '$\times 1/16$', ...
            'Units', 'normalized', ...
        ...
        % Comment out for final plot
        % title('Natural Frequency x Saturation parameter (\alpha)')

        % Use these to keep same scale on a bimat/stepped coupled plot
        % ylim([1.1e5 2.6e5]) % modes 18~22
        % ylim([1.5e4 6.4e4]) % modes 7~11
    ...
    function self = material(self,input)
        ...
        % depois fzer outr mÃ©todo sÃ³ pras seÃ§Ãµes bugadas e pra tapered, mudando o
        ...
        %tapered: definir funÃ§Ã£o com o handle e fzer o calculo de cada nÃ³ no metodo
        fields = {'E', 'rho'};
    ...
    function self = geometry(self,input)
        ...
        new_values = zeros(n_el, numel(fields)); % PrÃ©-aloca matriz para todos os campos
        
        masu_d = vertcat(self.data(input).d{:}); % Agrupa todos os d1, d2, d3
        masu_nu = vertcat(self.data(input).nu{:}); % Agrupa nu
        intervals = masu_d(:, end-1:end); % Assume que os intervalos sÃ£o os mesmos para d e nu
        
        % Preenche os valores corretamente entre os nÃ³s iniciais e finais
        for i = 1:size(intervals, 1)
            idx_range = intervals(i,1):intervals(i,2); % Ãndices do intervalo
    ...
    function self = multilayer(self,inp)
        ...
        % Inicializa os layers periÃ³dicos
        ...
        % Preenche os dados das camadas periÃ³dicas
        for i = 0:n_celulas-1
            offset = i * n_nodes;
